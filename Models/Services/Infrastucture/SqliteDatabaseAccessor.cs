...
namespace MyCourse.Models.Services.Infrastucture
{
    public class SqliteDatabaseAccessor : IDatabaseAccessor
    {
        ...
        public int QueryInsert(FormattableString formattableQuery)
        {
            ...
            using (var conn = new SqliteConnection("Data Source=Data/MyCourse.db"))
            {
                ...
                using (var cmd = new SqliteCommand(query, conn))
                {
                    //tramite il metodo AddRange aggiungo tutti i parametri recuperati dalla query
                    cmd.Parameters.AddRange(sqliteParameters);
                    var rowUpdated = Convert.ToInt32(cmd.ExecuteNonQuery());//per eseguire una query di INSERT,UPDATE o DELETE si usa il metodo ExecuteNonQuery
        ...
        //         using (var cmd = new SqliteCommand(query, conn))
        //         {
        //             cmd.Parameters.AddWithValue("title","Titolo Prova");
        //             cmd.Parameters.AddWithValue("author", "Autore Prova");
        //             var rowUpdated = Convert.ToInt32(cmd.ExecuteNonQuery());//per eseguire una query di INSERT,UPDATE o DELETE si usa il metodo ExecuteNonQuery
